...
import { saveBlocks, restoreBlocks, indexOfRegExp, makeDiscreetQuery, match } from '../utils.js'
...
class Template {
  ...
  /**
   * Render {{#IF}} statements. These statements take the form of:
   * `{{#IF|<cond>}}<main>{{#ENDIF}}`, or with an optional alternative block,
   * `{{#IF|<cond>}}<main>{{#ELSE}}<else>{{#ENDIF}}`. The `cond` condition can
   * either be the name of a named parameter (in which case it is evaluated as
   * `true` if it's given a parameter with that name), or an expression of
   * equality, as in `param=val` (in which case it is evaluated as `true` if
   * it's given a parameter with that name, and that parameter is equal to the
   * value `val`).
   * If the condition is true, the full statement is replaced by the `<main>`
   * element. If it is not true and you've provided an `<else>` element, that
   * will replace the full statement. If you haven't provided an `<else>`
   * statement, then the full statement will be removed altogether.
   * @param {string} str - The string to parse.
   * @param {{named: {}, ordered: []}} params - An object recording the
   *   parameters passed to this instance, as generated by the `parseTemplates`
   *   method.
   * @returns {string} - The original string rendered with all `#IF` statements
   *   evaluated and rendered accordingly.
   */

  static renderIfs (str, params) {
    const regex = /{{#IF\|([.\s\S]*?)}}([.\s\S]*?)({{#ELSIF}}([.\s\S]*?))?{{#ENDIF}}/im
    const ifs = match(str, regex)
    for (const i of ifs) {
      const elems = i.str.match(regex)
      if (!elems || elems.length < 5) continue
      const cond = elems[1].split('=')
      const test = cond.length < 1 ? false : cond.length === 1 ? params.named.hasOwnProperty(cond[0]) : params.named[cond[0]] === cond[1]
      const replace = test ? elems[2] : elems[4] || ''
      str = str.replace(i.str, replace)
    }
    return str
  }
