...
class Template {
   ...
   * `{{#IF|<cond>|<main>}}`, or with an optional else block,
   * `{{#IF|<cond>|<main>|<else>>}}`. The `cond` condition can either be the
   * name of a named parameter (in which case it is true if a value is provided
   * for that parameter), or an expression of equality, as `param=val` (in which
   * case it is true if a value if provided for that parameter, and the value
   * equals `val`). If the `cond` condition is true, then `<main>` is rendered.
   * If it is not true, and an `<else>` clause has been provided, then that
   * `<else>` clause is rendered. If one hasn't been provided, it is removed from
  ...
  static renderIfs (str, params) {
    ...
    for (const orig of ifs) {
      const elems = orig.substring(2, orig.length - 2).split('|').map(el => el.trim())
      const pair = elems.length > 1 ? elems[1].split('=').map(el => el.trim()) : []
      const condition = pair.length > 1
        ? params.named[pair[0]] && params.named[pair[0]] === pair[1]
        : pair.length > 0 ? params?.named.hasOwnProperty(pair[0]) : false
      if (condition && elems.length > 2) {
        str = str.replace(orig, elems[2])
      } else if (!condition && elems.length > 3) {
