...
describe('dnd5e', () => {
  ...
  describe('characterForm', () => {
    it('returns information to create the form', () => {
      ...
      expect(actual[0]).to.be.eql({ name: 'dnd5e-int', label: 'Intelligence', detail: 'Intelligence Ability Modifier', type: 'number', default: 0 })
      expect(actual[1]).to.be.eql({ name: 'dnd5e-arcana', label: 'Arcana', detail: 'Intelligence (Arcana) Modifier', type: 'number', default: 0 })
      expect(actual[2]).to.be.eql({ name: 'dnd5e-history', label: 'History', detail: 'Intelligence (History) Modifier', type: 'number', default: 0 })
      expect(actual[3]).to.be.eql({ name: 'dnd5e-nature', label: 'Nature', detail: 'Intelligence (Nature) Modifier', type: 'number', default: 0 })
      expect(actual[4]).to.be.eql({ name: 'dnd5e-religion', label: 'Religion', detail: 'Intelligence (Religion) Modifier', type: 'number', default: 0 })
    })

    it('populates the character\'s stats', () => {
      const actual = characterForm({ dnd5e: { int: 1, arcana: 2, history: 3, nature: 4, religion: 5 } })
      expect(actual).to.have.lengthOf(5)
      expect(actual[0]).to.be.eql({ name: 'dnd5e-int', label: 'Intelligence', detail: 'Intelligence Ability Modifier', type: 'number', default: 1 })
      expect(actual[1]).to.be.eql({ name: 'dnd5e-arcana', label: 'Arcana', detail: 'Intelligence (Arcana) Modifier', type: 'number', default: 2 })
      expect(actual[2]).to.be.eql({ name: 'dnd5e-history', label: 'History', detail: 'Intelligence (History) Modifier', type: 'number', default: 3 })
      expect(actual[3]).to.be.eql({ name: 'dnd5e-nature', label: 'Nature', detail: 'Intelligence (Nature) Modifier', type: 'number', default: 4 })
      expect(actual[4]).to.be.eql({ name: 'dnd5e-religion', label: 'Religion', detail: 'Intelligence (Religion) Modifier', type: 'number', default: 5 })
    })
  })
